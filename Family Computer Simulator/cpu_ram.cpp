...
void CpuBus::save(uint16_t addr, uint8_t data)
{
	if (addr < 0x2000) {
		...
		case 0x2002:
			...
			break;
		case 0x2003:
			Ppu2.writeOamaddr(data);
			break;
		case 0x2004:
			Ppu2.writeOamdata(data);
			break;
		case 0x2005:
			Ppu2.writeScroll(data);
			break;
		case 0x2006:
			Ppu2.writeAddr(data);
			break;
...
uint8_t CpuBus::load(uint16_t addr)
{
	if (addr < 0x2000) {
		...
		case 0x2001: //PPUMASK
			...
			break;
		case 0x2002:
			return Ppu2.getStatus();
		case 0x2003:
			qDebug("cannot read OAMADDR\n");
			break;
		case 0x2004:
			return Ppu2.getOamdata();
		case 0x2005:
...
uint8_t* CpuBus::get_page_ptr(uint8_t page_id) {
	...
	if (addr < 0x2000) {
		...
		if (cartridge.has_added_ram) {
			...
			qDebug() << "get_page_ptr, Cannot get page ptr of " << addr << "\n";
			abort();
		}
	}
	else {
