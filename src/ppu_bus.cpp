...
void PictureBus::save(uint16_t addr, uint8_t data) {
	if (addr <= 0x1fff) {
		...
		getNametableAddr(addr, nametable_id, nametable_addr);
		if (nametable_id == 1)
...
uint8_t PictureBus::load(uint16_t addr) {
	if (addr < 0x2000) {
		...
		qDebug() << "PictureBus::load, address should in 0-0x4000\n";
		return 0x00;
	}
}

void PictureBus::getNametableAddr(uint16_t addr, uint8_t& nametable_id, uint16_t& nametable_addr) {
	...
	case NAMETABLE_ONESCREEN_HIGHER:
		...
		break;
	default:
		qDebug() << "Error: Unsupported Nametable as " << cartridge.mapper_ptr->nametable_mirror << " \n";
		nametable_id = 0;
		nametable_addr = addr & 0x3ff;
		break;
	}
